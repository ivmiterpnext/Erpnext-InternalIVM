...
class TestFetchCustomerNameAndContact(FrappeTestCase):
		...
		contact = frappe.get_doc(
			{
				...
				"first_name": "Jane",
				"last_name": "Smith",
				"email_ids": [{"email_id": "jane@example.com", "is_primary": 1}],
			}
		).insert()
	...
	def test_contact_linked_to_multiple_customers(self):
		...
		customer1 = frappe.get_doc(
			{"doctype": "Customer", "customer_name": "Customer One", "customer_type": "Individual"}
		).insert()
		customer2 = frappe.get_doc(
			{"doctype": "Customer", "customer_name": "Customer Two", "customer_type": "Individual"}
		).insert()
		...
		contact = frappe.get_doc(
			{
				...
				"first_name": "Multi",
				"last_name": "Link",
				"email_ids": [{"email_id": "multi@example.com", "is_primary": 1}],
				"links": [
					{"link_doctype": "Customer", "link_name": customer1.name},
					{"link_doctype": "Customer", "link_name": customer2.name},
				],
			}
		).insert()
	...
	def test_multiple_contact_emails_same_contact(self):
		...
		# Create a Customer
		customer = frappe.get_doc(
			{"doctype": "Customer", "customer_name": "Multi Email Customer", "customer_type": "Individual"}
		).insert()
	...
	def test_case_sensitivity_email(self):
		...
		# Create a Customer
		customer = frappe.get_doc(
			{"doctype": "Customer", "customer_name": "Case Test Customer", "customer_type": "Individual"}
		).insert()
		...
		fetch_customer_name_and_contact("TEST@EXAMPLE.COM")
