...
class TestCustomEmailAccountSendAutoReply(FrappeTestCase):
	...
	def setUp(self):
		...
		self.email_account = frappe.get_doc(
			{"doctype": "Email Account", "email_id": f"test-{frappe.generate_hash(length=8)}@example.com"}
		).insert()
	...
	def test_single_communication_calls_super(self):
		...
		issue = frappe.get_doc({"doctype": "Issue", "subject": "Test Issue"}).insert()
		...
		with patch(
			"frappe.email.doctype.email_account.email_account.EmailAccount.send_auto_reply"
		) as mock_super:
	...
	def test_boundary_one_communication(self):
		...
		issue = frappe.get_doc({"doctype": "Issue", "subject": "Boundary Test"}).insert()
	...
	def test_boundary_two_communications(self):
		...
		# Create a reference document
		issue = frappe.get_doc({"doctype": "Issue", "subject": "Boundary Test 2"}).insert()
