...
# HubSpot ID field on CRM Organization.  Provisioned via the custom_field.json
# fixture as ``custom_hubspot_company_id`` on the CRM Organization DocType.
HUBSPOT_ID_FIELD = "custom_hubspot_company_id"


# ---------------------------------------------------------------------------
# Value transforms — each receives (doc, raw_hubspot_value) and mutates doc.
# ---------------------------------------------------------------------------
...
def _apply_organization_name(doc: Any, raw_value: Any) -> None:
    """Handle organization_name — requires rename if the doc already exists.

    The actual rename is performed in ``_sync_company`` after all field
    mapping; this transform is a no-op to prevent ``apply_field_map``
    from setting it directly (which would fail for existing docs since
    ``organization_name`` is the primary key / autoname field).
    """
    # Intentionally a no-op — handled in _sync_company via rename_doc.
    pass
...
COMPANY_TRANSFORMS: dict[str, Any] = {
    ...
    "organization_name": _apply_organization_name,
...
def _sync_company(hubspot_company_id: int | str, org_name: str) -> None:
    ...
    all_properties = list(COMPANY_FIELD_MAP.keys()) + COMPANY_ADDRESS_PROPERTIES
