...
def link_existing_customer_to_deal(crm_deal_name: str) -> str:
	...
	if deal.custom_customer and frappe.db.exists("Customer", deal.custom_customer):
		...
		frappe.logger(_LOG).info(
			f"Resolved existing Customer '{customer_name}' from custom_customer on deal {crm_deal_name}"
		)
...
def resolve_and_link_master_client(crm_deal_name: str) -> str | None:
	...
	frappe.db.set_value("Customer", deal.custom_customer, "custom_master_customer", master_customer_name)
	frappe.logger(_LOG).info(
...
def _link_contact_to_customer(contact_doc: Any, customer_name: str) -> None:
	...
	frappe.logger(_LOG).info(f"Linked Contact '{contact_doc.name}' to Customer '{customer_name}'")
