...
def _copy_flat_fields(doc: Any, mapping: dict[str, str]) -> dict[str, Any]:
	...
	return {dst: value for src, dst in mapping.items() if (value := doc.get(src)) is not None and value != ""}
...
def _create_project_for_location(
	...
	if location.hubspot_site_id and frappe.db.exists(
		"Project", {"custom_hubspot_deployment_site_id": location.hubspot_site_id}
	):
	...
	frappe.logger(_LOG).info(
		f"Created Deployment {project.name} from CRM Deal {deal.name} / Deployment Location {location_name}"
	)
