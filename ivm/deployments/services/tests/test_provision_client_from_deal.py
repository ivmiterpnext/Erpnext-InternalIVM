...
def _make_deal(organization=None, custom_customer=None, custom_master_organization=None,
               custom_master_customer=None, contacts=None):
...
class TestProvisionCustomerAndIcorpClient(ERPNextTestSuite):
    ...
    def test_creates_new_customer_when_no_match(self):
        ...
        with patch(_ICORP_POST, return_value={"contact": {"id": 111}}) as mock_post, \
             patch(_EXTRACT_ID, side_effect=[111, 222]):
            result = provision_customer_and_icorp_client(deal.name)

        self.assertTrue(frappe.db.exists("Customer", {"customer_name": org.organization_name}))
        self.assertEqual(result, frappe.db.get_value("Customer", {"customer_name": org.organization_name}, "name"))
        deal.reload()
    ...
    def test_icorp_client_id_stored_on_customer(self):
        ...
        with patch(_ICORP_POST, return_value={"contact": {"id": 111}}), \
...
class TestResolveAndLinkMasterClient(ERPNextTestSuite):
    ...
    def test_uses_existing_master_customer_field(self):
        ...
        self.assertEqual(frappe.db.get_value("Customer", customer.name, "custom_master_customer"), master.name)
    ...
    def test_creates_master_customer_from_master_organization(self):
        ...
        self.assertEqual(result, frappe.db.get_value("Customer", {"customer_name": master_org.organization_name}, "name"))
