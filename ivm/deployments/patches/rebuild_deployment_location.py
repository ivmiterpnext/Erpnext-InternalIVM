...
def execute():
	...
	count_result = frappe.db.sql("SELECT COUNT(*) FROM `tabDeployment Location`")
	...
	for table, dt_col, _dn_col in SYSTEM_TABLES:
		try:
			frappe.db.sql(f"DELETE FROM `{table}` WHERE `{dt_col}` = 'Deployment Location'")
		except Exception:
			pass
...
def _export_table_to_csv(export_dir, slug):
	...
	columns_result = frappe.db.sql("SHOW COLUMNS FROM `tabDeployment Location`")
	column_names = [row[0] for row in columns_result]

	records = frappe.db.sql("SELECT * FROM `tabDeployment Location`", as_dict=True)
