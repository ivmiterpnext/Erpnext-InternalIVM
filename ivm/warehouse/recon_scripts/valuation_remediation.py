...
def parse_inventory(path):
	...
	for row in ws.iter_rows(min_row=2, values_only=True):
		bay, _product_type, _entry_name, sid, qty = row[0], row[1], row[2], row[3], row[4]
...
def parse_prices(path):
	...
	for row in ws.iter_rows(min_row=3, values_only=True):
		_name, sid, price = row[0], row[1], row[2]
...
def find_gap_rows(sheet_stock, prices, bay_warehouses):
	...
	existing_items = set(frappe.get_all("Item", filters={"item_code": ["in", item_codes]}, pluck="item_code"))
...
def main():
	...
	log(
		f"  Updated: {item_updated}, Already correct: {item_already_correct}, Skipped (no price): {item_skipped_no_price}"
	)
	...
	if len(rows) <= 20:
		...
		for r in sorted(rows, key=lambda x: (x["item_code"], x["warehouse"])):
			log(
				f"    {r['item_code']} @ {r['warehouse']}: qty={r['qty']} (unchanged), rate -> {r['valuation_rate']}"
			)
	else:
