...
function injectBinsEditor(frm, cdt, cdn) {
	...
	function setNumberState($row, isStorage) {
		...
		if (isStorage) {
			$input
				.prop("disabled", false)
				.val($input.data("saved") || "")
				.css("color", "");
		} else {
			$input
				.data("saved", $input.val())
				.prop("disabled", true)
				.val("N/A")
				.css("color", "#aaa");
		}
    ...
    `);
		...
		$row.find(".ivm-remove-bin").on("click", () => {
			$row.remove();
			saveBins();
		});
	...
	$container.find(".ivm-add-bin").on("click", () => {
		addRow();
		saveBins();
	});
}
...
function renderBinsReadOnly($container, value, df) {
	...
	if (!bins.length) {
		$container.html(
			'<div class="text-muted small" style="padding:4px 0;">No bins configured</div>'
		);
		return;
	}

	let html = '<table class="table table-bordered table-sm mb-0" style="table-layout:fixed;">';
	html +=
		'<thead><tr><th style="width:33.33%;">Type</th><th style="width:33.33%;">#</th><th style="width:33.33%;">Size (Inches)</th></tr></thead><tbody>';
...
Object.assign(window, {
	setupMachineDetailGrids,
	injectBinsEditor,
	validateUniqueMachineNames,
	renderBinsReadOnly,
});
