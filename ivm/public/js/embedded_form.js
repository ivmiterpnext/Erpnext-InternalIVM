...
ivm.EmbeddedForm = class {
	...
	_register_auto_sync() {
		...
		if (!frm.__embedded_forms) {
			...
			frm.refresh = function (...args) {
				...
				frm.__embedded_forms.forEach(function (ef) {
					ef.render();
				});
				return result;
			};
	...
	_compute_section_col_classes(meta) {
		...
		// Last section
		const cols = col_breaks + 1;
		section_col_class[current_section] =
			cols <= 1
				? "col-sm-12"
				: cols === 2
				? "col-sm-6"
	...
	_render_fields(meta, form_container) {
		...
		meta.fields.forEach(function (df) {
			...
			if (state.pending_section_label !== null) {
				state.current_section = $('<div class="form-section"></div>').appendTo(
					form_container
				);
				if (state.pending_section_label) {
					state.current_section.append(
						`<div class="section-head">${state.pending_section_label}</div>`
					);
				}
				state.current_row = $('<div class="section-body"></div>').appendTo(
					state.current_section
				);
				state.current_column = $(
					`<div class="form-column ${state.col_class}"></div>`
				).appendTo(state.current_row);
	...
	_handle_tab_break(df, form_container, state) {
		...
		state.current_column = $(`<div class="form-column ${state.col_class}"></div>`).appendTo(
			state.current_row
		);
	}
	...
	_evaluate_depends_on(depends_on) {
		...
		try {
			...
			let condition = depends_on.startsWith("eval:") ? depends_on.substring(5) : depends_on;
	...
	_save_field_values() {
		for (let fieldname in this.field_controls) {
			try {
				...
				console.warn(`Error reading value for field "${fieldname}":`, e);
			}
		}
	}
