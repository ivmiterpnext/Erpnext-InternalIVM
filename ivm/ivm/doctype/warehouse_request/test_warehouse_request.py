...
from ivm.warehouse.services.pick_list import create_pick_list, add_item_to_pick_list
...
def _make_build_wr_with_pick_list_stock_entry(submit_stock_entry=True):
    ...
    se = make_stock_entry(
        item_code=item.name, qty=5, from_warehouse=WAREHOUSE, to_warehouse=TARGET_WAREHOUSE,
        company=COMPANY, stock_entry_type="Material Transfer", do_not_submit=True,
    )
...
class TestValidateCratedStatus(ERPNextTestSuite):
    ...
    def test_crated_with_draft_stock_entry_throws(self):
        wr, se, pl_name = _make_build_wr_with_pick_list_stock_entry(submit_stock_entry=False)
        wr.reload()
        wr.status = "Crated - Ready to Ship"
        with self.assertRaises(frappe.ValidationError) as ctx:
            wr.save(ignore_permissions=True)
    ...
    def test_crated_with_submitted_stock_entry_passes(self):
        wr, se, pl_name = _make_build_wr_with_pick_list_stock_entry(submit_stock_entry=True)
        wr.reload()
        wr.status = "Crated - Ready to Ship"
        wr.save(ignore_permissions=True)
...
class TestValidateClosedStatus(ERPNextTestSuite):
    ...
    def test_non_inventory_shipment_skips_check(self):
        wr = _make_warehouse_request(
            request_reason="Shipping Request", non_inventory_shipment=1, status="New",
        )
    ...
    def test_closed_draft_material_transfer_throws(self):
        wr, se = _make_shipping_wr_with_material_transfer(submit_stock_entry=False)
        wr.reload()
        wr.status = "Closed"
        with self.assertRaises(frappe.ValidationError) as ctx:
            wr.save(ignore_permissions=True)
    ...
    def test_closed_submitted_material_transfer_passes(self):
        wr, se = _make_shipping_wr_with_material_transfer(submit_stock_entry=True)
        wr.reload()
        with patch(_DN_PATCH_TARGET):
            wr.status = "Closed"
            wr.save(ignore_permissions=True)
