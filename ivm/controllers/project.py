...
class CustomProjectController(OriginalProjectController):
	...
	def update_delivery_and_install_contact_due(self):
		...
		if self.expedited_delivery:
			base_days = 9
			days = calculate_days(int(self.added_days), weekday, base_days)
			due_days = add_days(self.placement_agreement, days)
			self.graphic_design_approval_due = due_days
			self.delivery_and_install_contact_due_customs =due_days
			self.delivery_install_and_coi_requirements=due_days
			self.user_and_restriction_requirements_due = user_and_restriction_requirements
		else:
			base_days = 10
			days = calculate_days(int(self.added_days), weekday, base_days)
			due_days = add_days(self.placement_agreement, days)
			...
			base_days1 = 20
			days1= calculate_days(int(self.added_days), weekday, base_days1)
			self.graphic_design_approval_due = add_days(self.placement_agreement, days1)
...
# preparing date based on the placement agreement, base days and added days data #
def get_provide_planogram_base_date(date, base_days, added_days):
	try:
		no_of_days = base_days + added_days + get_no_days(date, base_days, added_days)
