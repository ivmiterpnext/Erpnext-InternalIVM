...
def sync_deal_activities(hubspot_deal_id: int | str, crm_deal_name: str) -> None:
	...
	for engagement_type in ALL_ENGAGEMENT_TYPES:
		try:
			...
			frappe.log_error(
				title=(f"HubSpot: failed to sync {engagement_type} for deal {hubspot_deal_id}"),
...
def _sync_attachments(
	...
	for file_id in file_ids:
		...
		try:
			...
			frappe.logger(_LOG).info(f"Attached file '{safe_name}' to CRM Deal {crm_deal_name}")
		except Exception:
			frappe.log_error(
...
def _sync_note(engagement_id: str, props: dict[str, Any], crm_deal_name: str) -> None:
	...
	if existing_name:
		if body:
			...
			frappe.logger(_LOG).info(f"Updated FCRM Note {existing_name} from HubSpot note {engagement_id}")
	...
	with _as_owner(props.get("hubspot_owner_id")) as owner_email:
		...
		frappe.logger(_LOG).info(f"Synced HubSpot note {engagement_id} → FCRM Note {doc.name}")
...
def _sync_call(engagement_id: str, props: dict[str, Any], crm_deal_name: str) -> None:
	...
	with _as_owner(props.get("hubspot_owner_id")) as owner_email:
		...
		frappe.logger(_LOG).info(f"Synced HubSpot call {engagement_id} → CRM Call Log {doc.name}")
...
def _create_calendar_note(
	...
	contact_email = props.get("hs_email_from_email") or props.get("hs_email_sender_email") or ""
...
def _sync_email(engagement_id: str, props: dict[str, Any], crm_deal_name: str) -> None:
	...
	comm_date = _parse_timestamp(props.get("hs_timestamp")) or _parse_timestamp(props.get("_createdAt"))
	...
	with _as_owner(props.get("hubspot_owner_id")) as owner_email:
		...
		frappe.logger(_LOG).info(f"Synced HubSpot email {engagement_id} → Communication {doc.name}")
...
def _sync_task(engagement_id: str, props: dict[str, Any], crm_deal_name: str) -> None:
	...
	if existing_name:
		...
		doc.save(ignore_permissions=True)
		_sync_attachments(props.get("hs_attachment_ids"), crm_deal_name)
		frappe.logger(_LOG).info(f"Updated CRM Task {existing_name} from HubSpot task {engagement_id}")
		return
	...
	with _as_owner(props.get("hubspot_owner_id")) as owner_email:
		...
		frappe.logger(_LOG).info(f"Synced HubSpot task {engagement_id} → CRM Task {doc.name}")
...
def _sync_meeting(
	...
	if existing_name:
		...
		doc.save(ignore_permissions=True)
		_sync_attachments(props.get("hs_attachment_ids"), crm_deal_name)
		frappe.logger(_LOG).info(f"Updated FCRM Note {existing_name} from HubSpot meeting {engagement_id}")
		return
	...
	with _as_owner(props.get("hubspot_owner_id")) as owner_email:
		...
		frappe.logger(_LOG).info(f"Synced HubSpot meeting {engagement_id} → FCRM Note {doc.name}")
