...
def execute(dry_run: bool = False) -> None:
	...
	try:
		...
		for idx, deal in enumerate(deals, start=1):
			...
			lines.append(f"[{idx}/{len(deals)}] {crm_deal_name} — {new} {label}, {existing} already existed")
...
def _count_existing_engagements(
	...
	rows = frappe.db.sql(
		f"SELECT COUNT(*) FROM `tab{doctype}` WHERE `{HUBSPOT_ENGAGEMENT_ID_FIELD}` IN %(ids)s",
		{"ids": engagement_ids},
	)
	return rows[0][0] if rows else 0
...
def _count_existing(crm_deal_name: str) -> int:
	return sum(
		[
			...
			frappe.db.count(
				"CRM Call Log", {"reference_doctype": "CRM Deal", "reference_docname": crm_deal_name}
			),
			frappe.db.count(
				"CRM Task", {"reference_doctype": "CRM Deal", "reference_docname": crm_deal_name}
			),
			frappe.db.count(
				"Communication", {"reference_doctype": "CRM Deal", "reference_name": crm_deal_name}
			),
		]
	)
