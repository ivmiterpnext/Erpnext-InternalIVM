...
def upsert_contact(
	...
	if contact_doc:
		...
		try:
			...
			frappe.db.commit()
			frappe.logger("hubspot").info(f"Created Contact {contact_doc.name} ({first_name} {last_name})")
...
def _find_existing_contact(
	...
	if hubspot_contact_id:
		name = frappe.db.get_value(
			"Contact",
			{HUBSPOT_ID_FIELD: hubspot_contact_id},
			"name",
		)
		if name:
	...
	for key, value in properties.items():
		...
		if df.fieldtype == "Link" and df.options:
			if not frappe.db.exists(df.options, value):
				frappe.logger("hubspot").warning(f"{df.options} '{value}' not found — skipping Contact.{key}")
				continue
