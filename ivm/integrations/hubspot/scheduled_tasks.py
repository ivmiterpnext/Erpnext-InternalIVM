...
from ivm.integrations.hubspot import activity_handler, api
...
def sync_inbound_emails() -> None:
	...
	deal_map: dict[str, str] = {d[HUBSPOT_DEAL_ID_FIELD]: d["name"] for d in open_deals}
	hubspot_ids = list(deal_map.keys())

	frappe.logger(_LOG).info(f"HubSpot inbound email sync: checking {len(hubspot_ids)} open deal(s)")
	...
	new_email_ids = [eid for eid in all_email_ids if f"<hubspot-email-{eid}>" not in synced_message_ids]

	frappe.logger(_LOG).info(
		f"HubSpot inbound email sync: {len(all_email_ids)} total emails, {len(new_email_ids)} new"
	)
