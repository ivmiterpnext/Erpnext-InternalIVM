...
class HubSpotRateLimitExhausted(Exception):
	def __init__(self, retry_after_seconds: float) -> None:
		...
		super().__init__(f"HubSpot rate limit hit. Suggested re-enqueue delay: {retry_after_seconds}s")
...
def _request(
	...
	*,
	...
	def send(attempt: int) -> requests.Response:
...
def get_associated_ids(
	...
	results = res.get("associations", {}).get(association_key, {}).get("results", [])
...
def download_file(file_id: str) -> tuple[str, bytes] | None:
	...
	try:
		...
		if not cdn_url:
			frappe.logger("hubspot").warning(f"No signed URL returned for HubSpot file {file_id}")
			return None
		...
		frappe.logger("hubspot").warning(
			f"Failed to download HubSpot file {file_id}: {frappe.get_traceback(with_context=False)}"
		)
		return None
...
def get_owner_email(owner_id: int | str) -> str | None:
	...
	try:
		...
		frappe.logger("hubspot").warning(f"Could not resolve HubSpot owner {owner_id}")
		return None
