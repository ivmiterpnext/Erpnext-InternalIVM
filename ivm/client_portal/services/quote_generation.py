...
def create_service_quote_from_deal(crm_deal):
	...
	if not primary_contact_doc:
		frappe.throw(f"CRM Deal {crm_deal} has no primary contact — cannot generate a Service Quote.")

	if not deal.deal_owner:
		frappe.throw(f"CRM Deal {crm_deal} has no deal owner set — cannot determine a sales representative.")
