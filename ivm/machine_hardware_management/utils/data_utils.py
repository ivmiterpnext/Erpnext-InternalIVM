...
def build_sort_params(order_by, sort_field_map: dict, *, add_tie_breaker=False):
	...
	if add_tie_breaker:
		tie_api = map_with_alias.get("name") or map_with_alias.get("id") or "Id"
		params += [
...
def _parse_order_by(order_by: str):
	...
	m = re.match(
		r"(?:(?P<table>[^.\s]+)\.)?(?P<field>[^.\s]+)(?:\s+(?P<dir>asc|desc))?$", clause, re.IGNORECASE
	)
	if not m:
		return None, None
...
def normalize_child_table_field(value, child_field):
	...
	for i, val in enumerate(values, start=1):
		rows.append({child_field: val, "idx": i})
