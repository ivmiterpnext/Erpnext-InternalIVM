...
from ivm.integrations.icorp import icorp_api_delete, icorp_api_get, icorp_api_post, icorp_api_put
from ivm.integrations.icorp.utils import api_data_to_frappe_dict, convert_fields_to_bool
...
class BaseVirtualDoctype(Document):
	...
	BOOL_FIELDS: ClassVar[list[str]] = []  # List of boolean fields in the doctype for conversion
	FIELD_MAP: ClassVar[
		dict[str, str]
	] = {}  # Mapping of Frappe field names to API field names. Frappe expects "name" for the primary key
	SORT_FIELD_MAP: ClassVar[dict[str, str]] = {}  # Mapping of sortable fields if different from FIELD_MAP
	...
	def get_list(cls, args):
		...
		cache_key = f"{cls.__name__.lower()}_list_cache_{sorted(params.items())!s}"
	...
	def db_insert(self, *args, **kwargs):
		...
		try:
			...
			data["created_date"] = datetime.now(UTC).isoformat()
	...
	def db_update(self, *args, **kwargs):
		...
		try:
			...
			data["modified_date"] = datetime.now(UTC).isoformat()
	...
	def get_count(cls, args):
		...
		params = cls.build_list_api_params(args)
		cache_key = f"{cls.__name__.lower()}_count_cache_{sorted(params.items())!s}"
		cached = frappe.cache().get_value(cache_key)
