...
frappe.ui.form.on("Board", {
	onload: function (frm) {
		...
		if (
			frm.doc.offline_sales_mode &&
			frm.offline_sales_mode_code_to_desc[frm.doc.offline_sales_mode]
		) {
			frm.set_value(
				"offline_sales_mode_description",
				frm.offline_sales_mode_code_to_desc[frm.doc.offline_sales_mode]
			);
		}
		...
		frappe.call({
			...
			callback: function (r) {
				if (r.message) {
					...
					if (
						frm.doc.primary_rfid_target_number_base_code &&
						frm.rfid_target_base_code_to_desc[
							frm.doc.primary_rfid_target_number_base_code
						]
					) {
					...
					if (
						frm.doc.secondary_rfid_target_number_base_code &&
						frm.rfid_target_base_code_to_desc[
							frm.doc.secondary_rfid_target_number_base_code
						]
					) {
					...
					if (
						frm.doc.setting3_rfid_target_number_base_code &&
						frm.rfid_target_base_code_to_desc[
							frm.doc.setting3_rfid_target_number_base_code
						]
					) {
					...
					if (
						frm.doc.setting4_rfid_target_number_base_code &&
						frm.rfid_target_base_code_to_desc[
							frm.doc.setting4_rfid_target_number_base_code
						]
					) {
					...
					if (
						frm.doc.setting5_rfid_target_number_base_code &&
						frm.rfid_target_base_code_to_desc[
							frm.doc.setting5_rfid_target_number_base_code
						]
					) {
		...
		frm.set_df_property(
			...
			"description",
			"Automatically set based on PROSE"
		);

		frm.set_query("board_manufacturer_id", function () {
			return {
				query: "ivm.machine_hardware_management.services.link_queries.get_board_manufacturer_link",
			};
		});

		frm.set_query("board_type_code", function () {
			return {
				query: "ivm.machine_hardware_management.services.link_queries.get_board_type_link",
			};
		});

		frm.set_query("hardware_availability_type_code", function () {
			return {
				query: "ivm.machine_hardware_management.services.link_queries.get_hardware_availability_type_link",
			};
		});
	...
	offline_sales_mode_description: function (frm) {
		if (
			frm.offline_sales_mode_desc_to_code &&
			frm.doc.offline_sales_mode_description in frm.offline_sales_mode_desc_to_code
		) {
			frm.set_value(
				"offline_sales_mode",
				frm.offline_sales_mode_desc_to_code[frm.doc.offline_sales_mode_description]
			);
		} else {
	...
	primary_rfid_target_number_base_description: function (frm) {
		if (
			frm.rfid_target_base_desc_to_code &&
			frm.doc.primary_rfid_target_number_base_description in
				frm.rfid_target_base_desc_to_code
		) {
			frm.set_value(
				"primary_target_number_base_code",
				frm.rfid_target_base_desc_to_code[
					frm.doc.primary_rfid_target_number_base_description
				]
			);
		} else {
	...
	secondary_rfid_target_number_base_description: function (frm) {
		if (
			frm.rfid_target_base_desc_to_code &&
			frm.doc.secondary_rfid_target_number_base_description in
				frm.rfid_target_base_desc_to_code
		) {
			frm.set_value(
				"secondary_target_number_base_code",
				frm.rfid_target_base_desc_to_code[
					frm.doc.secondary_rfid_target_number_base_description
				]
			);
		} else {
	...
	setting3_rfid_target_number_base_description: function (frm) {
		if (
			frm.rfid_target_base_desc_to_code &&
			frm.doc.setting3_rfid_target_number_base_description in
				frm.rfid_target_base_desc_to_code
		) {
			frm.set_value(
				"setting3_target_number_base_code",
				frm.rfid_target_base_desc_to_code[
					frm.doc.setting3_rfid_target_number_base_description
				]
			);
		} else {
	...
	setting4_rfid_target_number_base_description: function (frm) {
		if (
			frm.rfid_target_base_desc_to_code &&
			frm.doc.setting4_rfid_target_number_base_description in
				frm.rfid_target_base_desc_to_code
		) {
			frm.set_value(
				"setting4_target_number_base_code",
				frm.rfid_target_base_desc_to_code[
					frm.doc.setting4_rfid_target_number_base_description
				]
			);
		} else {
	...
	setting5_rfid_target_number_base_description: function (frm) {
		if (
			frm.rfid_target_base_desc_to_code &&
			frm.doc.setting5_rfid_target_number_base_description in
				frm.rfid_target_base_desc_to_code
		) {
			frm.set_value(
				"setting5_target_number_base_code",
				frm.rfid_target_base_desc_to_code[
					frm.doc.setting5_rfid_target_number_base_description
				]
			);
		} else {
		...
		frappe.db.get_value(
			...
			function (r) {
				...
				if (manufacturer_name === "VendNovation") {
					...
					frm.set_df_property(
						"is_pin_entry_enabled",
						"label",
						"Is Keypad ID Entry Enabled?"
					);
				} else {
