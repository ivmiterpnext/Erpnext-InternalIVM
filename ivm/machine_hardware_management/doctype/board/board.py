...
class Board(BaseVirtualDoctype):
	...
	BOOL_FIELDS: ClassVar[list[str]] = [
		"is_update_firmware",
		"is_update_connection",
		"is_update_rfid",
		"is_dhcp",
		"offline_vend_storage",
		"is_update_machine_motor_info",
		"is_pin_entry_enabled",
		"keypad_id_entry",
		"has_rfid_configuration",
		"primary_has_bit_reverse_feature",
		"secondary_has_bit_reverse_feature",
		"setting3_has_bit_reverse_feature",
		"setting4_has_bit_reverse_feature",
	...
	def _set_vendnovation_configurations(self):
		try:
			...
			set_attrs_from_dict(self, data)
			self.has_rfid_configuration = (
				1 if getattr(self, "board_rfid_configuration_id", None) not in (None, "", "null") else 0
			)

			endpoint = (
				f"SV/BoardVendnovationConfiguration/GetByBoardSerialNumber?SerialNumber={self.serial_number}"
			)
			response = icorp_api_get(endpoint)
			data = response.get("data", {})

			configs = sorted(data, key=lambda c: c.get("effective_date") or "", reverse=True)
...
def get_rfid_target_number_base_types():
	...
	options = [
		{"name": str(item.get("id")), "code": item.get("code"), "description": item.get("description")}
		for item in items
	]
	return options
...
def get_manufacturer_by_serial_number(board_serial_number):
	...
	try:
		...
		if result:
			return {"id": result.get("id"), "manufacturer_name": result.get("name")}
		return None
	except Exception as e:
		if hasattr(e, "response") and getattr(e.response, "status_code", None) == 404:
			return {"id": None, "manufacturer_name": "Invalid PROSE Number"}

		frappe.log_error(frappe.get_traceback(), "get_manufacturer_by_serial_number error")
		return {"id": None, "manufacturer_name": "Error"}
